...
Место на диске освобождает фоновая уборка (раз в `VM_GC_INTERVAL_MINUTES` минут, по умолчанию
шесть часов). В логе она отмечается строкой `Уборка: Report(...)`, а вкладка «Корзина» в
админке показывает итог последнего прохода и умеет запустить его кнопкой:

```bash
c logs server | grep -i уборка
du -sh /var/lib/docker/volumes/vitz-music_media/_data/*   # что сколько занимает по разделам
```

Удалённый трек живёт в корзине `VM_TRASH_RETENTION_DAYS` дней — это надгробие, по которому
плеер узнаёт об удалении на синхронизации. Срок меньше, чем машина может простоять без сети,
ставить не стоит: телефон, пропустивший надгробие, оставит трек у себя навсегда.
